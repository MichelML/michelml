      ...
      <Grid container spacing={40}>
        {posts.map((post) => (
          <Grid item key={post.name} sm={6} md={6} lg={6}>
            ...
            <Link href={`/blog/${post.name.replace(/\s/g, "").toLowerCase()}`}>
              <CardMedia
                className={classes.cardMedia}
                image={assetUrl("static/postimages/" + post.cleanName + ".svg")}
                title={post.name}
              />
              </Link>
              <CardContent className={classes.cardContent}>
                ...
                <Typography gutterBottom variant="subheading" component="h6" color="textSecondary">
